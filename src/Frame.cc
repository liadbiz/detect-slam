    ...
    for(auto s:bbinfo)
    {
      ...
      for(int i = 0;i<N;i++)
      {
        if(mvpMapPoints[i])
        {
          ...
          const cv::Mat pc_ = mRcw * p_ + mtcw;
          const double &pcx_ = pc_.at<double>(0);
          const double &pcy_ = pc_.at<double>(1);
          const double &pcz_ = pc_.at<double>(2);

          const double invz_ = 1.0 / pcz_;
          const double px = fx*pcx_*invz_ + cx;
          const double py = fy*pcy_*invz_ + cy;
